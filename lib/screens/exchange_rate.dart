...
class _ExchangeRateState extends State<ExchangeRate> {
  ...
  String _result="";
  final List<String>_currencies=['USD','EUR','GBP','INR','JPY','AUD','BOB','BZD','COP','AWG'];
  Future<void> fetchExchangeRate()async{
    //final apiKey=dotenv.env['EXCHANGE-API-KEY'];
    final url=Uri.parse("https://v6.exchangerate-api.com/v6/2e37190133ded1be935f3570/latest/$_baseCurrency");
    try{
  ...
  void convertCurrency() {
    ...
    setState(() {
      _result = "$amount $_baseCurrency=${convertedAmount.toStringAsFixed(
          2)}$_targetCurrency";
    });
  }
  @override
      ...
      Container(
        ...
        child: Column(
          children: [
                  ...
                  value: _baseCurrency,
                    items: _currencies.map((String currency){
                      return DropdownMenuItem(
                        value: currency,
                          child: Text(currency,style: TextStyle(fontSize: 24),),
                      );
                    }).toList(),
                    onChanged: (value){
                    setState(() {
            ...
            SizedBox(height: 30),
            Text(":Converted Value:",style: TextStyle(fontSize: 20,fontWeight: FontWeight.bold),),
            SizedBox(height: 10),
            Text(
              _result,
